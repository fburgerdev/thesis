...
\begin{myparagraph}
    ...
    Here, the term \defterm{extended} means that when restricting
    such a microbundle
...
\begin{myparagraph}
    % cone
    For a topological space $X$, we define the \defterm{cone} $CX$ of $X$ by
    \[ \cyl{X} / X \cross \{1\} \]
    and the \defterm{mapping cone} $B \sqcup_f CA$ over a map $f: A \to B$ by
    \[ B \sqcup CA / \sim \]
    where $(a, 0) \sim b \iff f(a) = b$.

    % cylinder
    Similarly, we define the \defterm{cylinder} $MX$ of $X$ by
    \[ \cyl{X} \]
    and the \defterm{mapping cylinder} $B \sqcup_f MA$ over a map $f: A \to B$ by
    \[ B \sqcup MA / \sim \]
    where $(a, 0) \sim b \iff f(a) = b$.
...
\begin{myproof}
    ...
    due to \myintref{induced::trivial} and the retraction
