...
\begin{myproof}
    ...
    it follows that $\ind{(\iota \circ f)}\bb'$ is isomorphic to $\ind{const}\bb'$ and hence trivial (\intref{induced::const}).
    ...
    From \intref{induced::trivial} and the retraction $(a, t) \mapsto (a, 1)$ it follows that $\ind{r}\bb\restr{A \times [\half, 1]}$ is trivial.
    Since $A$ is paracompact and by \intref{microbundle::paracompact}, there exists a map
    ...
    by removing a closed subset of $E(\ind{r}\bb\restr{A \times [\half, 1]})$ if necessary and applying \intref{microbundle::total}.
